...
the uniform and non-uniform mesh displacements, 
...
we present results for the proxy non-uniform mesh displacement.
...
We now show results for a form with a nonlinear function inside the weak form.
...
\clearpage
\subsection{Results for Non-Uniform Mesh Displacement}
...
the solution RB approximation error is smaller or greater than the (M)DEIM error.

The conclusion of the experiments is that if the solution RB error is smaller than the (M)DEIM error,
...
When the solution RB error is above the (M)DEIM error, the trucation technique 
...
\clearpage
\subsection{Non-Uniform Mesh: N-MDEIM by Mode Evaluation}
