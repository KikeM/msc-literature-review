...
% Hence, we explore an alternative approach, 
During the simulation of the FOM, 
we collect snapshots of the nonlinear term and compress them with the same  
...
\begin{table}[h]
    \centering
    \caption{Parameter space sampling for nonlinear term reconstruction.}
...
\subsubsection{Non-Hierarchical Basis}
...
\subsubsection{Fourier Basis Exploration}
We investigate further the non-hierarchical character of the N-MDEIM basis.
The snapshots were obtained from evaluations 
of the nonlinear term with reduced basis elements,
\begin{equation}
    \begin{split}
...
\begin{equation}
    \psi_h(x) = \sum_{k} \psi_k \varphi_{k}(x),
\end{equation}
which leads to the following exact expression for each of the entries of the nonlinear term,
\begin{equation}
    \begin{split}
        \qty[\Ah{N}\left(\psi_{h}\right)]_{i,j}
        &= b_0 \sum_k \psi_k \int_{0}^{L(t^{n+1})} \varphi_{k} \grad \varphi_j \varphi_i \dd x.
    \end{split}
\end{equation}
...
This detailed inspection of the matrix elements has not clarified our doubts.
For the assembly of the nonlinear operator during the FOM integration, 
...
The remaining degrees of freedom are the coefficient values $\psi_k$.
...
We kick-off with a naive approach, selecting the following elements of the basis:
\begin{equation}
    \psi_q = \qty{\cos(2 \pi \omega_q x) \, : \, \omega_q \in \qty{1, \ldots, 20}}.
\end{equation}
\begin{figure}[h]
    \centering
    \includegraphics[width=1\columnwidth]{research_project/piston/figures/mdeim_certification/fourier_basis.png}
    \caption{Fourier basis for the $[0,1]$ interval for $\omega_q \in \qty{1, 2, 5, 10}$.}
    \label{fig:fourier_basis}
\end{figure}

We are going to carry out two experiments:
\begin{itemize}
    \item (A) Run this basis through the SVD and analyze its spectrum decay. 
    We would expect to find no decay at all, since the basis is orthogonal.
    \item (B) Compute snapshots of the nonlinear operator term with this basis
    and run them through the SVD as well. 
    Only one parameter will be selected, and the snapshots will be collected
    as time varies.
    Two alternative strategies can be used:
        \begin{itemize}
            \item (B1) Collect snapshots for each basis element and then compress.
            \item (B2) Collect for all basis elements and then compress. 
        \end{itemize}
