...
% -----------------------------------------------------------------------------
\subsection{A Naive Galerkin Approach}
...
We do so to motivate the operator reduction procedures we will include, 
especially regarding the operators.
...
we have a linear mapping between the problem-based functions~$\psi_i(x)$ and the nodal basis functions.
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
where $\vb{X}_{h}$ and $\vb{F}_{h}$ stand for each of the FOM operators (matrix and vector respectively).
...
\begin{subequations}
    ...
    \begin{align}
        ...
        \\
        \vb{K}_{N}^{n+1} &= \Ah{M} + \dt \left[\Ah{A} + \Ah{B} \right. 
        \nonumber 
        \\
...
\begin{enumerate}
    ...
    \end{equation*}
\end{enumerate}
These two facts imply that the projection of the operators, Equation~\eqref{eq:1d_rom_linear_system_operators}, does not break the original constraint of homogeneous boundary conditions;
and that the linear expansion of the solution~$\hat{u}_N(x)$ in the span of~$V_N$, Equation~\eqref{eq:1d_rom_burgers_rb_expansion}, is always true.
\mytodo{There was a paper which discussed this kind of details regarding homogeneous boundary projection.}

Once the reduced homogeneous solution~$\hat{u}_N(x)$ is obtained, it can be brought back to the original space~$V_h$ via Equation~\eqref{eq:1d_rom_burgers_projection_relation}, and then add on top of it the FE representation of the Dirichlet lifting, to obtain the final solution,
\begin{equation}
...
By means of a nested POD as we explained in Section~\ref{sec:1d_rom_burgers_basis_construction_nested}, if we replace the solution snapshots with operator snapshots, we can build the static and problem-dependent basis $A_{h,q}$.
...
The indices $\mathcal{I}_a$ are selected during the offline stage, according to error reduction arguments of the separable form \eqref{eq:1d_rom_burgers_system_approximation},
\begin{equation}
...
\subsubsection{Sacrifical Mode}
We can use two ROMs to estimate the error with respect to the FOM solution.

\begin{equation}
    \norm{u_h - \rbV_{N} u_N} = \norm{u_h - \rbV_{N+1} u_{N+1} + \rbV_{N+1} u_{N+1} - \rbV_{N} u_{N}}.
\end{equation}
By the triangle inequality, we get
\begin{equation}
    \norm{u_h - \rbV_{N} u_N} \leq \norm{u_h - \rbV_{N+1} u_{N+1}} + \norm{\rbV_{N+1} u_{N+1} - \rbV_{N} u_{N}}.
\end{equation}
...
\begin{equation}
    e_N \leq e_{N+1} + \norm{\rbV_{N+1} u_{N+1} - \rbV_{N} u_{N}}.
\end{equation}
With sufficient ROB\footnote{Reduced Order Basis} elements, it should be safe to assert that the error made 
with an extra mode should be smaller or equal to the one made without it,
\begin{equation}
    e_{N+1} \leq e_{N}.
\end{equation}
...
\subsubsection*{ROMs error: Implementation Details}
The error between the two ROMs,
\begin{equation}
    \norm{\rbV_{N+1} u_{N+1} - \rbV_{N} u_{N}},
\end{equation}
can be expressed as a sum in terms of the ROB elements.
Due to the hierarchical character of the ROB elements, $\rbV_{N+1}$ contains the same elements 
up to $N$ as $\rbV_{N}$.
...
\begin{equation}
    \begin{split}
        \norm{\rbV_{N+1} u_{N+1} - \rbV_{N} u_{N}} 
        = \\ 
        \norm{u^{N+1}_{N+1} \psi_{N+1} + \sum_{j}^{N} (u^{N+1}_{j} - u^{N}_{j}) \psi_j}
    \end{split}
\end{equation}
The difference between ROM coefficients $(u^{N+1}_{j} - u^{N}_{j})$ should be relatively small, 
since it represents the difference between the two coefficients associated to the same mode.
If they were notably different, it would mean that the dynamics between the original ROM and the one 
carrying the sacrificial mode are different.
Since the basis has a hierarchical character, this effect is unlikely to happen: 
adding a mode should only refine the solution, not change how the previous modes are scaled.
They are not strictly the same because the ROM system entries change if more modes are added to the basis,
but again, they should do so in a way that somehow preserves dynamics.
