...
\end{equation}
This function usually shows exponential decay, or a sharp drop beyond a given number of elements.
...
we could project each element of the operator basis $A_{h, q}$ to obtain an expression for the reduced operator, 
\begin{equation}
    \begin{split}
...
takes as input in its third argument an extrapolation of the velocity $u^{*}$.
This extrapolation lives in the same function space as the solution:
from Equation~(\ref{eq:u_star_approximation}),
it is a linear combination of past solutions in time.

The general strategy to reduce this operator 
is to collect and compress the operator snapshots 
...
However, if the $u^{*}$ extrapolations are going to be expressed in terms of the RB solution modes,
...
Therefore, the trilinear operator modes can be built with two strategies,
\begin{itemize}
...
\end{itemize}
These strategies are summarize in Table~\ref{tab:summary_trilinear_strategies}.

The \mbox{$u^{*}$-restricted} requires a slight modification of the nested POD algorithm.
...
\begin{figure}[h]
    ...
    The parameter space is sampled, for each parameter and RB mode 
    snaphsots are collected and compressed in time (red dots).
    This provides a collateral basis for each parameter.
    These bases are finally compressed into one, 
    which summarizes the changes in time, parameters and RB modes.}
...
if the RB modes are obtained, e.g., from experimental data.

\begin{table}[h]
    \centering
    \caption{Trilinear operator reduction strategies}
    \begin{tabular}{@{}lccc@{}}
    \toprule
                       & Op. assembly      & 3rd argument          & Nested levels           \\ \midrule
    $u^{*}$-general    & FOM simulation    & Extrapolation $u^{*}$ & 2 (param., time)        \\
    $u^{*}$-restricted & Ind. offline step & RB solution modes     & 3 (param., modes, time) \\ \bottomrule
    \end{tabular}
