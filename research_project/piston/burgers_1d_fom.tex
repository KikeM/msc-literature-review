...
\mytodo{Outer boundary: What is the physical process that allows me to not apply a boundary conditon.}
\mytodo{Outer boundary: Ignoring the + sign means that no inflow is permitted.}
\mytodo{Outer boundary: Constant Riemann invariant outside of the domain.}
\mytodo{Outer boundary: Clarify what is happening at the model outer boundary.}
\mytodo{Outer boundary: More detailed section about the boundary condition.}
\begin{equation}
...
\mytodo{Outer boundary: Natural outflow condition means evaluating the weak form at the boundary.
...
\end{equation}
For such a simple domain, the mesh velocity quantity is a linear interpolation 
...
when we comment upon the reduction of the linear operators.
...
\begin{subequations}
    \begin{align}
        ...
        \pdv{J_{\alemap}}{t} &= L'(t),
        \\
        J_{\alemap} \nabla \cdot w_m &= L(t) \frac{L'(t)}{L(t)} = L'(t),
    \end{align}
\end{subequations}
which proves the identity.
Equation~\eqref{eq:jacobian_derivative} could be part of our problem if we did not have access 
to an explicit expression for the jacobian, and we wanted to solve our problem in a fixed reference domain.
...
is the relation between the piston motion and the static speed of sound\footnote{
...
\mytodo{Nomenclature: is this the right way to formulate a weak form?}
\begin{subequations}
    \begin{align}
...
\subsubsection{Semi-Discrete Weak Formulation for Both Schemes}
...
In the FE~context with Lagrangian basis elements, 
