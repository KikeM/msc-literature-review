...
Because the system is homogeneous.}
\begin{equation}
    \begin{vmatrix}
...
\mytodo{What is the physical process that allows me to not apply a boundary conditon.}
\mytodo{Ignoring the + sign means that no inflow is permitted.}
\mytodo{Constant Riemann invariant outside of the domain.}
\mytodo{Clarify what is happening at the model outer boundary.}
\mytodo{More detailed section about the boundary condition.}
\begin{equation}
...
\mytodo{Explain the limitations of a constant viscosity term.}
...
\end{equation}
\mytodo{The symbol for mesh velocity is very similar as the oscillating frequency.}
...
\mytodo{What about the convergence rates?} 
