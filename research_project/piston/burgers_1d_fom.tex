...
\mytodo{FOM: Explain the limitations of a constant viscosity term.}
...
\end{equation}
\mytodo{Nomenclature: the symbol for mesh velocity $w$ is very similar as the oscillating frequency $\omega$.}
...
\mytodo{Add text: define ALE map and mesh velocity for the piston problem.}
...
\subsubsection{BDF-1}
\mytodo{Time-discretization: update BDF-1 proof, it is done with the heat equation. Should I move this to an appendix? Should I simplify this to be equation-independent?}
...
\mytodo{Time-discretization: include BDF-2 discrete scheme.}
