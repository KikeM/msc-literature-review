...
Our very first task is to determine an acceptable range to sample each parameter.
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
The symbols $t^{(\cdot)}_{+}$ and $t^{(\cdot)}_{-}$ denote the first positive and the second\footnote{We take the second negative peak and not the first one to remove any possible distortion due to the initial transient from rest to movement.} negative peaks in the velocity function at their respective locations.

The closer $\eta$ is to one, the more linear the response is.
As it tends to zero, the response is become nonlinear, since a shock wave is building up at the outflow;
\begin{align*}
...
However, points with low values of $a_0$ belong to the set without wiggles. 
Therefore, it must be the combination of them which explains the existence of wiggles.

Indeed, when we look at the correlation between~$u_p$ and~$\eta$, we observe a linear trend between them: the higher the forcing, the stronger the nonlinearity in the response.
And clearly, beyond a threshold in the forcing, the system seems to break and most solutions contain wiggles.
This critical value~$u_M$ seems to be in the neighborhood of~$u_M \sim 0.4$. 

With this information available (and after running some tests) we choose the following ranges for our parameters:
\begin{table}[h]
    \centering
